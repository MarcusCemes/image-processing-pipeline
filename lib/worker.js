...
let debug = data => process.send({code: "DEBUG", data: data});
...
async function process_image(image_path, options) {
  ...
  // Just copy the file if it's an svg or gif
  if (metadata.format === 'svg' || metadata.format === 'gif') {
    const read_stream = fs.createReadStream(image_path);
      if (!fs.existsSync(export_parse.dir)) mkDirSyncRecursive(export_parse.dir);
      read_stream.once('error', () => { throw {reason: "Failed to open read_stream for image"} } );
      return await new Promise(resolve => { read_stream.pipe(
        fs.createWriteStream(export_path).once('finish', () => {
          read_stream.destroy();
          resolve(new Export(image_parse.name, image_parse.ext, 'single', universalPath(relative_path)));
        }));
      });
  }
  ...
  raw_bytes += image_buffer.info.size;
  ...
  for (let i = 0; i !== number_exports; i++) {
    ...
    if (should_export) {
      ...
      raw_bytes += resized_image_buffer.info.size;
      ...
      try {
        ...
        // Encode to format and save the file, saving the written bytes to a variable
        const webp_uncompressed_size = (await sharp(resized_image_buffer.data, raw_info).webp().toFile(webp_path)).size;
        const fallback_uncompressed_size = (await sharp(resized_image_buffer.data, raw_info).toFormat(metadata.format).toFile(fallback_path)).size;

        raw_bytes += webp_uncompressed_size + fallback_uncompressed_size;

        // Apply image optization
        if (config.optimize === true) {
          if (metadata.format === 'png') {

            await imagemin([fallback_path], export_parse.dir, { use: [ pngquant({ quality: "70-90", floyd: 1, speed: 1 }) ] });
          } else if (metadata.format === 'jpeg') {
            await imagemin([fallback_path], export_parse.dir, { use: [ mozjpeg({ quality: 80 }) ] });
          }
        }

        // Store statistical data
        compressed_bytes += fs.statSync(fallback_path).size + webp_uncompressed_size;
        uncompressed_bytes += webp_uncompressed_size + fallback_uncompressed_size;
        ...
        to_push['webp'] = universalPath(path.relative(output_path, webp_path));
        to_push['fallback'] = universalPath(path.relative(output_path, fallback_path));
        current_export['default'] === true ? to_push['default'] = true : null;

        exported_sizes.push(to_push);
  ...
  for (const seg of segments) {
    ...
    if (bypass || (bypass = !fs.existsSync(new_path))) {
      try {
        ...
        if (err.code !== 'EEXIST') throw err; // Ignore if directory was made by another worker
      };
    }
  }
}

function universalPath(p) {
