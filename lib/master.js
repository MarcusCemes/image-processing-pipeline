...
async function Master(passed_options) {
  ...
  if (!image_list) {
    ...
    if (ctrl_result.errors.length === 0) {
      ...
      fs.writeJsonSync(path.join(options.output, 'errors.json'), ctrl_result.errors);
    ...
    if (options.manifest) {
      ...
      fs.writeJSONSync(path.join(options.output, 'manifest.json'), ctrl_result.completed);
...
async function verify_paths() {
  ...
  try {
    ...
    fs.unlinkSync(path.join(options.output, bg));  // Delete the file
...
function generate_no_images_response() {
  ...
  fs.writeJsonSync(manifest_path, {exports:[]}); // Write an empty manifest
  return new Response(
    'NOIMG',
